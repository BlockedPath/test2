...
const statusElement =
	document.querySelector<HTMLParagraphElement>("#connection-status");
...
if (!statusElement || !lobbyContainer || !pixiContainer || !debugElement) {
	throw new Error(
		"HTML missing #connection-status / #lobby-root / #pixi-root / #snapshot-output.",
	);
}

const endpoint =
	(import.meta as unknown as { env: { VITE_COLYSEUS_ENDPOINT?: string } }).env
...
muteBtn.textContent =
	localStorage.getItem("br_muted") === "1" ? "Unmute" : "Mute";
...
const bulletNodes = new Map<string, Graphics>();
...
function getBulletNode(id: string): Graphics {
	let g = bulletNodes.get(id);
	if (!g) {
		g = new Graphics();
		g.beginFill(0xffff00);
		g.drawCircle(0, 0, 3);
		g.endFill();
		bulletNodes.set(id, g);
		world.addChild(g);
	}
	return g;
}
...
devBtn.style.cssText =
	"margin:8px auto;display:block;padding:6px 12px;background:#d73;color:#fff;border:none;cursor:pointer;";
		...
		if (latestState?.lobby) {
			if (wantsToShoot) {
				statusElement!.textContent =
					"Can't shoot in lobby — Ready and Start with 2 (dev) first";
			}
			return;
		}
...
function tick(now: number): void {
	if (latestState && !latestState.lobby) {
		...
		// Bullets (tracers) — server moves at 400 u/s, ~20 ticks lifetime
		for (const [bid, bullet] of latestState.bullets) {
			const node = getBulletNode(bid);
			node.x = bullet.x;
			node.y = bullet.y;
		}
		for (const bid of [...bulletNodes.keys()]) {
			if (!latestState.bullets.has(bid)) {
				const g = bulletNodes.get(bid);
				if (g) {
					world.removeChild(g);
					bulletNodes.delete(bid);
				}
			}
		}
			...
			debugElement!.textContent = JSON.stringify(
				{
					...
					own: { x: Math.round(cx), y: Math.round(cy), hp: own.hp },
				},
				null,
				2,
			);
